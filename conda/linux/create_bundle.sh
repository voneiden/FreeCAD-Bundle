...
packages="freecad=0.21.2 occt vtk python=${PYTHON_VERSION} blas=*=openblas numpy \
          matplotlib-base scipy sympy pandas six pyyaml pycollada lxml \
          xlutils olefile requests blinker opencv qt.py nine docutils \
          opencamlib calculix ifcopenshell appimage-updater-bridge openssl<3.2"
#if [[ "$ARCH" = "x86_64" ]]; then
...
sed -i 's/Exec=FreeCAD/Exec=AppRun/g' AppDir/org.freecadweb.FreeCAD.desktop
